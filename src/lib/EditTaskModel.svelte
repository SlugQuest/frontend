    ...
    <Dialog.Content>
      ...
      {#if !task.IsAllDay}
        <div class="grid grid-cols-4 items-center gap-4">
          ...
          <Input id="taskStartTime" class="col-span-3" type="datetime-local" bind:value={task.StartTime} />
        </div>
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="taskEndTime" class="text-right">End Date:</Label>
          <Input id="taskEndTime" class="col-span-3" type="datetime-local" bind:value={task.EndTime} />
        </div>
        {#if badDatetime}
      ...
      <div class="grid grid-cols-4 items-center gap-4">
        ...
        <Select.Root bind:selected={difficulty} onSelectedChange={(value) => {
          if (task !== undefined) {
